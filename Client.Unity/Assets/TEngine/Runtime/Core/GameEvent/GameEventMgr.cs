...
namespace TEngine
{
    ...
    public class GameEventMgr : IMemory
    {
        ...
        public void AddEvent<TEvent>(Action<TEvent> handler) where TEvent : IEvent
        {
            if (GameEvent.AddEventListener(handler))
            {
                AddEventImp(RuntimeId.ToRuntimeId(typeof(TEvent).FullName), handler);
            }
        }
