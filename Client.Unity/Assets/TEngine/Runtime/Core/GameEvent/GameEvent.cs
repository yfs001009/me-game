...
namespace TEngine
{
    ...
    public class GameEvent
    {
        ...
        private static int GetEventType<TEvent>() where TEvent : IEvent
        {
            return RuntimeId.ToRuntimeId(typeof(TEvent).FullName);
        }
        ...
        public static bool AddEventListener<TEvent>(Action<TEvent> handler) where TEvent : IEvent
        {
            return _eventMgr.Dispatcher.AddEventListener(GetEventType<TEvent>(), handler);
        }
        ...
        /// <param name="handler">事件处理回调。</param>
        /// <typeparam name="TEvent">事件类型。</typeparam>
        public static void RemoveEventListener<TEvent>(Action<TEvent> handler) where TEvent : IEvent
        {
            _eventMgr.Dispatcher.RemoveEventListener(GetEventType<TEvent>(), handler);
        }
        ...
        /// <typeparam name="TEvent">事件类型。</typeparam>
        public static void Send<TEvent>(TEvent eventData) where TEvent : IEvent
        {
            _eventMgr.Dispatcher.Send(GetEventType<TEvent>(), eventData);
        }
